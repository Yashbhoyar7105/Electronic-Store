...
public class Helper {
    ...
    public static <U,V> PageableResponse<V> getPagebleResponse(Page<U> page,Class<V> type){
        ...
        List<V> dtoduser = entity.stream().map(object -> new ModelMapper().map(object,type)).collect(Collectors.toList());

        PageableResponse<V> response=new PageableResponse<>();
        response.setContent(dtoduser);
        response.setPageNumber(page.getNumber());
        response.setPageSize(page.getSize());
        response.setTotalElements(page.getNumberOfElements());
        response.setTotalNoPages(page.getTotalPages());
